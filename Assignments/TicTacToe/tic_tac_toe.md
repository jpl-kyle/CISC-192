# Challenges faced
This assignment required a lot of focus and background knowledge, so I had trouble getting started. I overcame this by taking a step back and solving each step at a time, and recognizing that I should begin with the `playGame();` function. I finally recognized that I needed to use it after making the game loop in main, and realizing that if I wanted an option to play again, I would need to call the `playGame();` function again. Quite a lot of my mistakes came from not putting semicolons where they needed to, mistyping logic (like `((!cin >> row >> column))` vs. `(!(cin >> row >> column))`), and because of that, I would frequently run into infinite loops. or errors just reminding me to put semicolons. I definitely need more practice with C++.
